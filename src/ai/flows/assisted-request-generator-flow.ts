 ...
 *
 * - generateAssistedRequest - A function that helps users formulate a clear and detailed inquiry message for Massimo.
...
const AssistedRequestGeneratorOutputSchema = z.object({
  generatedMessage: z
    .string()
    .describe('The AI-assisted, detailed inquiry message for Massimo, ready to be sent.'),
  suggestedNextSteps: z
    .array(z.string())
    .describe('A list of suggestions for the user on what additional information might be helpful for Massimo, or what to expect next.'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `Sei un assistente AI per Massimo Artigiano, un artigiano specializzato in:
...
Il tuo compito è aiutare un potenziale cliente a formulare una richiesta di contatto chiara e dettagliata per Massimo.
L'obiettivo è assicurare che Massimo abbia tutte le informazioni necessarie per fornire una risposta efficace e un preventivo accurato.
...
Genera un messaggio di richiesta dettagliato e professionale che Massimo possa ricevere. Il messaggio dovrebbe:
1. Salutare Massimo in modo cortese.
...
Inoltre, fornisci un elenco di "suggestedNextSteps" al cliente, indicando quali ulteriori informazioni potrebbero essere utili a Massimo o cosa aspettarsi dopo l'invio della richiesta. Ogni elemento della lista deve essere una frase concisa.
...
- Massimo la contatterà a breve per discutere i dettagli.
`,
});
